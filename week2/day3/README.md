...
This file contains SQL NULL function concepts and practice queries using Employees, Orders, and Products tables.
...
- `ISNULL(column, value)` → Replaces NULL with a given value
- `COALESCE(a, b, c)` → Returns the first non-null value
- `NULLIF(a, b)` → Returns NULL if both values are equal
- `IS NULL` / `IS NOT NULL` → Used to check NULL values
